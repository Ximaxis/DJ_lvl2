...
class Command(BaseCommand):
    def handle(self, *args, **options):
        # test_products = Products.objects.filter(Q(category__name='Мужская одежда') | Q(category__name='Женская одежда')).select_related()
        ...
        for orderitem in test_orderss:
            print(
                f"{orderitem.action_order:2}: заказ №{orderitem.pk:3}: {orderitem.product.title:15}: скидка {abs(orderitem.total_price):6.2f} руб. | {orderitem.order.updated - orderitem.order.created}"
            )
